  ...
  switch (check.method) {
    ...
    case "image_metadata": {
      ...
      if (hasUndersized || hasUnparseable) {
        // Veto: set_score to 0 on geopolitical_rigor
        return {
          charts: imgs.length,
          results,
          hasUndersized,
    ...
    case "score_figure_svg": {
      ...
      //    and unjustly drag down geopolitical_rigor. Exclude by viewBox signature and by
...
function evaluateRound3(fused, rubric, mechanicalResults) {
  ...
  // We use: analytical_depth mechanical ≥ 2.0 (not completely dead) AND language_quality has disclaimer
  const logicConsistMech = fused["analytical_depth"]?.mechanical ?? 0;
